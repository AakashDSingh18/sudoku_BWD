...
function newBoard() {
  let boardEl = document.getElementById('sudoku-board');
  boardEl.innerHTML = '';
  for (let i = 0; i < 9; i++) {
  for (let j = 0; j < 9; j++){
...
function setMessage(msg) {
  ...
  el.textContent = msg;
  el.className = (msg.length!==0) ? 'error-msg' : '';
}

// Starting point of the game
function play(){
  let st= document.getElementsByClassName('btn-play');
  st[0].style.display= "none";

  let subt= document.getElementsByClassName('subtitle');
  subt[0].innerHTML= `Fill in the grid to check the answer`;

  let wrap1 = document.getElementById('sudoku-board');
  wrap1.style.display= "grid";

  let wrap2 = document.getElementsByClassName('board-wrapper');
  wrap2[0].style.display= "flex";

  let nump= document.getElementById('numpad');
  nump.style.display= "flex";

  let buttons= document.getElementsByClassName('actions');
  buttons[0].style.display= "flex"; 

  let messages= document.getElementById('message');
  messages.style.display= "flex";

  let timers= document.getElementsByClassName('mistakes');
  timers[0].style.display= "flex";
  newGame();  
}

function triggerWin(){
  let win_time= document.getElementById('message')
